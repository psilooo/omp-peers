...
import type { PeerBinding } from './peers/binding.js';
...
function parseVersion(raw: unknown): [number, number, number] | undefined {
...
function isSupported(version: [number, number, number]): boolean {
  const [major, minor, patch] = version;
  if (major < MIN_MAJOR || major >= MAX_MAJOR_EXCLUSIVE) return false;
...
export default function peersExtension(pi: ExtensionHostLike): void {
  ...
  try {
    ...
    if (typeof pi.logger?.warn !== 'function') return;
    version = parseVersion(pi.pi?.VERSION);
  } catch {
    version = undefined;
  }
  if (version === undefined || !isSupported(version)) return;

  const on = (name: string, handler: (event: unknown, ctx: CommandContextLike) => unknown): void => {
    try {
      pi.on(name, (event: unknown, ctx: CommandContextLike) => {
        try {
