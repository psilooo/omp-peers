...
import { registerPeerTools, type PeerToolDeps } from '../tools.js';
import type { PeerTodo } from '../types.js';
import { ensureStateRoots, peerEndpoint, validateUnixEndpoint, type StateRoots } from '../store/paths.js';
import {
...
import { formatBeatAge, readPeerRecord, removeOwnRecord, scanPeers, writeOwnRecord, type PeerScan } from './presence.js';
...
export interface PeerBinding {
  ...
  onCommitPoint(ctx: CommandContextLike): void; // local input/command, peer-tool execution, context, before_agent_start
  onContext(event: unknown, ctx: CommandContextLike): { messages: RosterMessage[] } | undefined;
  // Extension-only wiring for the bounded activity/todo feed that statusSnapshot renders.
...
export function createPeerBinding(pi: ExtensionHostLike): PeerBinding {
  // Origin latch: classified exactly once at the first session_start.
  let originKind: SessionKind | undefined;
  ...
  let identity: { pid: number; instance: string; token: string } | undefined;
  let roots: StateRoots | undefined;
  let record: PeerRecordV2 | undefined;
  /** Last session-derived requested name handed to resolveName. */
  let lastNameRequested: string | undefined;
  let activeHandle: ServerHandle | undefined;
  let closedHandle: ServerHandle | undefined;
  ...
  let timers: ManagedTimers | undefined;
  ...
  async function safeClose(handle: ServerHandle | undefined): Promise<void> {
    if (handle === undefined || closedHandle === handle) return;
    closedHandle = handle;
    try {
      await handle.close();
    } catch {
  ...
  async function removeEndpointIfOurs(): Promise<void> {
    if (roots === undefined || identity === undefined) return;
    ...
    if (uid !== undefined && stats.uid !== uid) return;
    await rm(endpoint, { force: true });
  }

  async function verifySocket(endpoint: string): Promise<void> {
    const stats = await lstat(endpoint);
    if (!stats.isSocket()) throw new Error('the peer endpoint is not a socket');
    const uid = process.getuid?.();
    if (uid !== undefined && stats.uid !== uid) throw new Error('the peer endpoint is owned by another user');
    await chmod(endpoint, 0o600);
  }

  function notArmedText(): string {
  ...
  function keyOf(ctx: CommandContextLike | undefined): string {
    ...
    try {
      ...
      for (const getter of directGetters) {
        const value = (manager as unknown as Record<string, unknown>)[getter];
        if (typeof value === 'function') {
          const result = (value as () => unknown).call(manager);
          if (typeof result === 'string' && result !== '') path = result;
        }
        if (path !== '') break;
      }
      if (path === '') {
  ...
  function commitTransition(): void {
    if (closed) return;
    transcriptEpoch += 1;
    if (record !== undefined && lastCtx !== undefined) scheduleNameRefresh();
    try {
   ...
   */
  function requestedNameFor(
    ctx: CommandContextLike | undefined,
    project: string,
   ...
   */
  async function refreshName(fresh: PeerScan): Promise<void> {
    const bindingAtStart = bindingEpoch;
    const current = record;
    const currentRoots = roots;
    const currentIdentity = identity;
    if (closed || current === undefined || currentRoots === undefined || currentIdentity === undefined) return;
    ...
    if (resolved.name === current.name) return;
    if (closed || bindingAtStart !== bindingEpoch) return;
    try {
   ...
   */
  function refreshNameAtCommit(): void {
    if (closed || !armed || transitionPending || originKind !== 'top-level') return;
    if (record === undefined || identity === undefined) return;
    const requested = requestedNameFor(lastCtx, record.project, identity);
    if (requested === lastNameRequested) return;
    lastNameRequested = requested;
    scheduleNameRefresh();
  }
  ...
  function commitPoint(ctx: CommandContextLike | undefined, resetHop: boolean): void {
    ...
    refreshNameAtCommit();
    if (originKind !== 'top-level' || !transitionPending || oldKey === null) return;
  ...
  function overlayStatus(base: StatusSnapshot, fields: string[] | undefined): StatusSnapshot {
    const wants = (field: string): boolean => fields === undefined || fields.includes(field);
    const out: StatusSnapshot = { busy: base.busy };
    ...
    if (wants('activity')) {
      ...
      if (text !== undefined && text !== '') out.activity = sanitizeDisplay(text, MAX_STATUS_ACTIVITY_BYTES);
    }
    if (wants('todos')) {
   ...
   */
  async function fanOutStatus(rows: RosterRow[]): Promise<RosterRow[]> {
    const deps = depsRef;
    if (rows.length === 0 || deps === undefined) return rows;
    const out = rows.map((row) => ({ ...row }));
    const deadline = Date.now() + STATUS_BUDGET_MS;
    let cursor = 0;
    let exhausted = false;
    const detailFrom = (result: OutboundResult): NonNullable<RosterRow['detail']> | undefined => {
      if (!isSuccessCode(result.code) || result.status === undefined) return undefined;
      const detail: NonNullable<RosterRow['detail']> = {};
      ...
      if (result.status.todos !== undefined) detail.todoCount = result.status.todos.length;
    ...
    const worker = async (): Promise<void> => {
      for (;;) {
        ...
        const winner = await Promise.race([
          statusOf(deps, out[index].name, STATUS_FIELDS, { deadlineAt: deadline }).then(
        ...
        if (winner.kind === 'done') {
          ...
          if (detail !== undefined) out[index].detail = detail;
          continue;
        }
        if (winner.kind === 'skip') continue;
        exhausted = true;
        return;
      }
    };
  ...
  function buildDelivery(): HostDelivery {
    ...
    const delivery: HostDelivery = {
      captureEpoch: () => ({ binding: bindingEpoch, transcript: transcriptEpoch }),
      isEpochValid: (captured) =>
        captured.binding === bindingEpoch && captured.transcript === transcriptEpoch,
      ...
      reserveHeld: () => {
        if (!base.reserveHeld()) return false;
        heldCount += 1;
        return true;
      },
      releaseHeld: () => {
  ...
  function missingCapability(ctx: CommandContextLike): string | undefined {
  ...
  function toolNames(view: unknown): string[] {
    ...
    if (view instanceof Map) {
      const names: string[] = [];
      for (const key of view.keys()) if (typeof key === 'string') names.push(key);
      return names;
    }
    if (typeof view === 'object' && view !== null) return Object.keys(view);
    return [];
  }

  function toolSurfacesConverged(): boolean {
    let live: string[];
    let active: string[];
    try {
  ...
  function startConvergence(ctx: CommandContextLike): void {
  ...
  function buildRecord(
    ...
    self: { pid: number; instance: string; token: string },
    startedAt: number,
    scan: PeerScan
  ): PeerRecordV2 {
  ...
  async function arm(ctx: CommandContextLike): Promise<void> {
    ...
    arming = true;
    const bindingAtStart = bindingEpoch;
    const stale = (): boolean => closed || bindingAtStart !== bindingEpoch;
    ...
    try {
      ...
      if (managed === undefined) throw new Error('managed timers are unavailable');
      ...
      activeHandle = handle;
      const ready = await Promise.race([handle.ready, shutdownSignal.then(() => 'shutdown' as const)]);
      ...
      if (verify === undefined) throw new Error('the presence record failed read-back verification');
      if (stale()) {
        try {
          await removeOwnRecord(stateRoots, self.pid, self.instance);
        } catch {
      ...
      record = verify;
      cachedPeers = scan.routable.filter(
        (row) => row.pid !== self.pid || row.instance !== self.instance
      );
      depsRef = deps;
      ...
      if (!contextHookRegistered) {
        ...
        try {
          pi.on('context', (event: unknown, eventCtx: CommandContextLike) => {
            try {
  ...
  function onSessionStart(ctx: CommandContextLike): void {
    ...
    if (originKind !== 'top-level') return;
    if (armed || arming || failed || toolsRegistered) return;
    const missing = missingCapability(ctx);
    if (missing !== undefined) {
      fail(`host capability ${missing} is missing`);
      return;
    }
    const toolDeps: PeerToolDeps = { send, request, status };
    try {
  ...
  function onBeforeTransition(_kind: 'switch' | 'branch' | 'tree', ctx: CommandContextLike): void {
    if (closed || originKind !== 'top-level') return;
    lastCtx = ctx;
    oldKey = keyOf(ctx);
  ...
  function onTransition(kind: 'switch' | 'branch' | 'tree', _event: unknown, ctx: CommandContextLike): void {
    if (closed || originKind !== 'top-level') return;
    lastCtx = ctx;
    if (kind === 'tree') {
      // The tree post-event is its own commit point.
      if (transitionPending) commitTransition();
      return;
    }
  ...
  function onCommitPoint(ctx: CommandContextLike): void {
  ...
  function onActivity(kind: ActivityKind, event: unknown, ctx: CommandContextLike): void {
    if (closed || originKind !== 'top-level') return;
    lastCtx = ctx;
  ...
  async function snapshot(): Promise<PeersSnapshot> {
  ...
  function hopFor(to: string, replyTo: string | undefined): number {
    if (lastInboundFrom === undefined) return 0;
    if ((replyTo !== undefined && replyTo !== '') || to === lastInboundFrom) return lastInboundHop;
    return lastInboundHop + 1;
  }

  async function send(to: string, body: string, opts: { replyTo?: string }): Promise<OutboundResult> {
    commitPoint(lastCtx, false);
    const gate = gateResult();
    if (gate !== undefined) return gate;
    const deps = depsRef;
    if (deps === undefined) return { code: 'host_unavailable', detail: notArmedText() };
  ...
  async function request(to: string, body: string, opts: { timeoutMs?: number }): Promise<OutboundResult> {
    commitPoint(lastCtx, false);
    const gate = gateResult();
    if (gate !== undefined) return gate;
    const deps = depsRef;
  ...
  async function status(to: string): Promise<OutboundResult> {
    commitPoint(lastCtx, false);
    const gate = gateResult();
    if (gate !== undefined) return gate;
    const deps = depsRef;
