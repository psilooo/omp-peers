...
import { formatBeatAge, readPeerRecord, removeOwnRecord, removeOwnRecordSync, scanPeers, writeOwnRecord, type PeerScan } from './presence.js';
...
export interface PeerBinding {
  ...
  onCommitPoint(ctx: CommandContextLike, human: boolean): void; // human entries reset hops; peer-sourced entries keep them
...
export function createPeerBinding(pi: ExtensionHostLike): PeerBinding {
  ...
  function onCommitPoint(ctx: CommandContextLike, human: boolean): void {
   ...
   */
  function hopForRecord(record: PeerRecordV2): number {
