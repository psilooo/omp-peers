...
async function recordFileOk(path: string): Promise<boolean> {
  let stats;
  try {
...
type EntryOutcome =
  | { kind: 'routable'; record: PeerRecordV2 }
...
export async function writeOwnRecord(roots: StateRoots, record: PeerRecordV2): Promise<void> {
 ...
 * scan.
 */
export async function scanPeers(
  roots: StateRoots,
  self: PeerIdentity,
  now: number = Date.now()
): Promise<PeerScan> {
  const routable: PeerRecordV2[] = [];
  const incompatible: Array<{ pid: number; version: number }> = [];
  let handle;
  try {
 ...
 */
export async function readPeerRecord(
  roots: StateRoots,
  pid: number,
  instance: string,
  now: number = Date.now()
): Promise<PeerRecordV2 | undefined> {
  if (!Number.isInteger(pid) || pid < 1 || pid > PID_MAX) {
    return undefined;
  }
 ...
 */
export async function removeOwnRecord(roots: StateRoots, pid: number, instance: string): Promise<void> {
  if (!Number.isInteger(pid) || pid < 1 || pid > PID_MAX) {
    return;
  }
  if (!isCanonicalInstance(instance)) {
