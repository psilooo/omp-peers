...
export async function resolveWorkspaceScope(cwd) {
...
export function scopeId(scopeKey) {
...
async function findGitDir(start) {
    ...
    for (;;) {
        ...
        if (stats?.isFile()) {
            ...
            if (text === undefined || !text.startsWith('gitdir:'))
                return undefined;
...
async function readSmallText(path) {
    let handle;
    try {
        ...
        if (bytesRead > MAX_POINTER_BYTES)
            return undefined;
...
async function canonicalOr(path) {
    try {
