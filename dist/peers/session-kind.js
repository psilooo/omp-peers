 ...
 */
export function classifySession(manager) {
    ...
    for (const entries of lists) {
        for (const entry of entries) {
            if (asRecord(entry)?.['type'] === 'session_init')
                return 'nested';
        }
    }
    ...
    if (typeof readFile === 'function') {
        try {
            const value = readFile.call(manager);
    ...
    if (file !== undefined && file.endsWith('.jsonl') && file !== '') {
        try {
            if (existsSync(`${dirname(file)}.jsonl`))
                return 'nested';
        }
        catch {
    ...
    if (!entriesReadable || !idValid || !fileValid)
        return 'unknown';
    return 'top-level';
}
