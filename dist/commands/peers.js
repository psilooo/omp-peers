 ...
 */
import { MAX_STATUS_ACTIVITY_BYTES, MAX_STATUS_TEXT_BYTES, PROJECT_MAX_BYTES, } from '../peers/protocol.js';
...
function rowFields(row, selfName) {
    const fields = [
    ...
    if (row.name === selfName)
        fields.push('you');
    return fields;
}
/** `` `name` · project · working · beat 3s ago · model ... · you `` */
export function formatPeerLine(row, selfName) {
...
export function formatPeersText(snap, now) {
    ...
    if (selfRow !== undefined) {
        ...
        lines.push(`\`${sanitizeDisplay(snap.self.name, PROJECT_MAX_BYTES)}\` · ${sanitizeDisplay(snap.self.project, PROJECT_MAX_BYTES)} · you`);
    }
    ...
    if (sorted.length === 0 && selfRow === undefined) {
        ...
        for (const row of sorted)
            lines.push(formatPeerLine(row, snap.self.name));
    }
        ...
        pi.registerCommand('peers', {
            ...
            handler: async (_args, ctx) => {
                try {
                    ...
                    if (typeof select === 'function' && ctx.mode === 'tui' && snap.rows.length > 0) {
                        try {
                            ...
                            if (typeof picked === 'string' && picked !== '') {
                                const row = snap.rows.find((candidate) => sanitizeDisplay(candidate.name, PROJECT_MAX_BYTES) === picked);
                                notify(ctx, row !== undefined ? formatPeerLine(row, snap.self.name) : formatPeersText(snap, now), 'info');
                            }
                            return;
                        }
                        catch {
                    ...
                    const reason = sanitizeDisplay(err instanceof Error ? err.message : String(err), MAX_STATUS_TEXT_BYTES);
